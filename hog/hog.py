...
def roll_dice(num_rolls, dice=six_sided):
    ...
    for i in range(num_rolls):
        ...
        if outcome == 1:
            sow_sad = True
        point += outcome
    return 1 if sow_sad else point
...
def tail_points(opponent_score):
    ...
    """
    ones, opponent_score = opponent_score % 10, opponent_score // 10
...
def perfect_square(n):
    """Return whether n is a perfect square or not."""
...
def is_always_roll(strategy, goal=GOAL):
    ...
    False
    """
    n_first_roll = strategy(0, 0)
    for score in range(goal):
        for opponent_score in range(goal):
            if strategy(score, opponent_score) != n_first_roll:
                return False
    return True
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    for num_dice in range(1, 11):
        ...
        avg_score = avg_dice(num_dice, dice)
